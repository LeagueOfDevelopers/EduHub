        ...
        IEventConsumer<ReviewReceivedEvent>
    {
        ...
        public void Consume(CourseFinishedEvent @event)
        {
            _distributor.NotifyGroup(@event.GroupId,
                new CourseFinishedNotification(@event.GroupTitle, @event.TeacherName));
        ...
        public void Consume(ReviewReceivedEvent @event)
        {
            _distributor.NotifyTeacher(@event.GroupId,
                new ReviewReceivedNotification(@event.GroupTitle, @event.ReviewerName, @event.ReviewType));
            _eventRepository.AddEvent(new Event(@event));
        }
        ...
        {
            _distributor.NotifyGroup(@event.GroupId,
                new TeacherFoundNotification(@event.TeacherName, @event.GroupTitle));
            _eventRepository.AddEvent(new Event(@event));
        }
    }
}